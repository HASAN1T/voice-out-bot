...
# الإعدادات
# =============================
BOT_TOKEN = os.getenv('BOT_TOKEN')   # ← غيّر هذا عند التشغيل المحلي
YOUR_CHAT_ID = os.getenv('YOUR_CHAT_ID')
WEBHOOK_URL = os.getenv('WEBHOOK_URL')  # إذا وُجد → وضع Webhook، وإلا → وضع محلي

if not BOT_TOKEN or BOT_TOKEN == 'YOUR_LOCAL_BOT_TOKEN':
    raise ValueError("❌ يجب تعيين BOT_TOKEN (في code محليًا أو كمتغير بيئة)")
...
def notify_startup(mode="محلي"):
    ...
    if YOUR_CHAT_ID and WEBHOOK_URL:  # إرسال رسالة تأكيد فقط على Render
        try:
            bot.send_message(int(YOUR_CHAT_ID), f"✅ تم التشغيل على Render عبر Webhook!")
        except Exception as e:
            print(f"⚠️ لا يمكن إرسال رسالة التأكيد: {e}")
...
def handle_choice(call):
    ...
    bot.edit_message_text("يتم المعالجة... قد يستغرق 30–60 ثانية.", chat_id, call.message.id)
